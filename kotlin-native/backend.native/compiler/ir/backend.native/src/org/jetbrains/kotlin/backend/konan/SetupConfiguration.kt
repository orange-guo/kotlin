...
fun CompilerConfiguration.setupFromArguments(arguments: K2NativeCompilerArguments) = with(KonanConfigKeys) {
    ...
    if (arguments.lightDebugDeprecated) {
        report(CompilerMessageSeverity.WARNING,
    ...
    putIfNotNull(LIGHT_DEBUG, when (val it = arguments.lightDebugString) {
        ...
        null -> null
            ...
            report(CompilerMessageSeverity.ERROR, "Unsupported -Xadd-light-debug= value: $it. Possible values are 'enable'/'disable'")
            null
        }
    })
    putIfNotNull(GENERATE_DEBUG_TRAMPOLINE, when (val it = arguments.generateDebugTrampolineString) {
        ...
        null -> null
            ...
            report(CompilerMessageSeverity.ERROR, "Unsupported -Xg-generate-debug-tramboline= value: $it. Possible values are 'enable'/'disable'")
            null
        }
    })
    ...
    val memoryModelFromArgument = when (arguments.memoryModel) {
        ...
        null -> null
            ...
            report(CompilerMessageSeverity.ERROR, "Unsupported memory model ${arguments.memoryModel}")
            null
        }
    }
    ...
    if (arguments.friendModules != null)
        put(FRIEND_MODULES, arguments.friendModules!!.split(File.pathSeparator).filterNot(kotlin.String::isEmpty))

    if (arguments.refinesPaths != null)
        put(REFINES_MODULES, arguments.refinesPaths!!.filterNot(kotlin.String::isEmpty))
    ...
    if (libraryToAddToCache != null && !arguments.outputName.isNullOrEmpty())
        report(CompilerMessageSeverity.ERROR, "${K2NativeCompilerArguments.ADD_CACHE} already implicitly sets output file name")
    ...
    put(DESTROY_RUNTIME_MODE, when (arguments.destroyRuntimeMode) {
        ...
        "on-shutdown" -> DestroyRuntimeMode.ON_SHUTDOWN
            ...
            this@setupFromArguments.report(CompilerMessageSeverity.ERROR, "Unsupported destroy runtime mode ${arguments.destroyRuntimeMode}")
    ...
    putIfNotNull(GARBAGE_COLLECTOR, when (arguments.gc) {
        ...
        "cms" -> GC.CONCURRENT_MARK_AND_SWEEP
            ...
            report(CompilerMessageSeverity.ERROR, "Unsupported GC ${arguments.gc}")
            null
        }
    })
    putIfNotNull(PROPERTY_LAZY_INITIALIZATION, when (arguments.propertyLazyInitialization) {
        ...
        "disable" -> false
            ...
            report(CompilerMessageSeverity.ERROR, "Expected 'enable' or 'disable' for lazy property initialization")
            false
        }
    })
    putIfNotNull(ALLOCATION_MODE, when (arguments.allocator) {
        ...
        "mimalloc" -> AllocationMode.MIMALLOC
            ...
            report(CompilerMessageSeverity.ERROR, "Expected 'std' or 'mimalloc' for allocator")
            AllocationMode.STD
        }
    })
    putIfNotNull(WORKER_EXCEPTION_HANDLING, when (arguments.workerExceptionHandling) {
        ...
        "use-hook" -> WorkerExceptionHandling.USE_HOOK
            ...
            report(CompilerMessageSeverity.ERROR, "Unsupported worker exception handling mode ${arguments.workerExceptionHandling}")
    ...
    put(LAZY_IR_FOR_CACHES, when (arguments.lazyIrForCaches) {
        ...
        "disable" -> false
            ...
            report(CompilerMessageSeverity.ERROR, "Expected 'enable' or 'disable' for lazy IR usage for cached libraries")
            false
        }
    })
    ...
    putIfNotNull(LLVM_VARIANT, when (val variant = arguments.llvmVariant) {
        ...
        null -> null
            ...
            if (!file.exists) {
                report(CompilerMessageSeverity.ERROR, "`-Xllvm-variant` should be `user`, `dev` or an absolute path. Got: $variant")
                null
            } else {
...
private fun parsePreLinkCachesValue(
    ...
    null -> null
        ...
        configuration.report(CompilerMessageSeverity.ERROR, "Unsupported `-Xpre-link-caches` value: $value. Possible values are 'enable'/'disable'")
        null
    }
}
...
private fun parseLibraryToAddToCache(
    ...
    return if (input != null && !outputKind.isCache) {
        configuration.report(CompilerMessageSeverity.ERROR, "${K2NativeCompilerArguments.ADD_CACHE} can't be used when not producing cache")
        null
    } else {
        input
    }
...
fun parseBinaryOptions(
    ...
    return keyValuePairs.mapNotNull { (key, value) ->
        ...
        if (option == null) {
            configuration.report(CompilerMessageSeverity.STRONG_WARNING, "Unknown binary option '$key'")
            null
        } else {
...
private fun <T : Any> parseBinaryOption(
    ...
    return if (value == null) {
        configuration.report(CompilerMessageSeverity.STRONG_WARNING, "Unknown value '$valueName' of binary option '${option.name}'. " +
...
private fun parseBundleId(
    ...
    return if (argumentValue != null && outputKind != CompilerOutputKind.FRAMEWORK) {
        configuration.report(CompilerMessageSeverity.STRONG_WARNING, "Setting a bundle ID is only supported when producing a framework " +
                "but the compiler is producing ${outputKind.name.lowercase()}")
        null
    } else {
        argumentValue
