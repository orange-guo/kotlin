...
class FirJsKlibBackendFacade(
    ...
    private fun actualize(inputArtifact: IrBackendInput.JsIrBackendInput) {
        val dependentFragments = mutableListOf<IrModuleFragment>()

        lateinit var mainModuleFragment: IrModuleFragment
        for ((index, irModuleFragment) in inputArtifact.irModuleFragments.withIndex()) {
            if (index < inputArtifact.irModuleFragments.size - 1) {
                dependentFragments.add(irModuleFragment)
            } else {
                mainModuleFragment = irModuleFragment
            }
        }

        IrActualizer.actualize(mainModuleFragment, inputArtifact.symbolTable, dependentFragments)
    }
}