...
object CompanionIncorrectlyUnboundedWhenUsedAsLHSCallChecker : CallChecker {
    override fun check(resolvedCall: ResolvedCall<*>, reportOn: PsiElement, context: CallCheckerContext) {
        val callableReference = resolvedCall.call.callElement.parent as? KtCallableReferenceExpression ?: return
        val explicitReceiver = resolvedCall.call.explicitReceiver as? ClassQualifier ?: return
        val dispatchReceiver = resolvedCall.dispatchReceiver ?: return

        if (dispatchReceiver != explicitReceiver.classValueReceiver) return
        ...
        if (referencedClass == explicitReceiver.descriptor) return
...
private fun isReferenceToShortFormCompanion(
    ...
    if (context.trace.bindingContext[BindingContext.SHORT_REFERENCE_TO_COMPANION_OBJECT, simpleReference] == null) return false
    return true
}
