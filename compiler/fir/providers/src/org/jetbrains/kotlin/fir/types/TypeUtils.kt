...
private fun FirTypeRef.hideLocalTypeIfNeeded(
    ...
    return withReplacedConeType(coneTypeSafe<ConeKotlinType>()?.approximateToOnlySupertype(session))
}
...
private fun ConeKotlinType.approximateToOnlySupertype(session: FirSession): ConeKotlinType? {
    ...
    if (resultTypeArguments.isNotEmpty() && resultTypeArguments.size == typeArguments.size) {
        ...
        return ConeSubstitutorByMap(substitution, session).substituteOrSelf(this)
    }
    return result
}
