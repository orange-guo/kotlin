...
import org.jetbrains.kotlin.name.JsStandardClassIds.Callables.JsDefinedExternally
import org.jetbrains.kotlin.name.JsStandardClassIds.Callables.JsNoImpl
...
object FirJsExternalChecker : FirBasicDeclarationChecker() {
    ...
    override fun check(declaration: FirDeclaration, context: CheckerContext, reporter: DiagnosticReporter) {
        ...
        if (
            ...
            container?.let { it is FirClass && it.classKind == ClassKind.INTERFACE } == true
        ) {
    ...
    private val FirTypeRef.isConstructorCall: Boolean
        get() = source?.psi is KtSuperTypeCallEntry || source?.lighterASTNode?.tokenType == KtNodeTypes.SUPER_TYPE_CALL_ENTRY
