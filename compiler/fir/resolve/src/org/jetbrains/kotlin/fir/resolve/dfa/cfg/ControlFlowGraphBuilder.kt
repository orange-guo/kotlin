...
class ControlFlowGraphBuilder {
    ...
    fun returnExpressionsOfAnonymousFunction(function: FirAnonymousFunction): Collection<FirExpression>? {
        ...
        nonDirectJumps[exitNode].mapNotNullTo(returnValues) { it.returnExpression() }
        // fun() { throw } -> Nothing-returning stub
        // TODO: this doesn't work properly if the entire function is dead
        return if (returnValues.isEmpty() && exitNode.isDead) listOf(FirStub) else returnValues
    }
    ...
    private fun popClassGraph(): ControlFlowGraph {
        ...
        addEdge(lastInPlaceExit, exitNode, preferredKind = EdgeKind.CfgForward)
