...
class SerializedIrFileFingerprint private constructor(val pathFingerprint: FingerprintHash, val fileFingerprint: FingerprintHash) {
    companion object {
        private const val FILE_HASH_SEPARATOR = "="

        fun fromString(s: String): SerializedIrFileFingerprint? {
            val hashes = s.split(FILE_HASH_SEPARATOR).mapNotNull(FingerprintHash::fromString)
            return hashes.takeIf { it.size == 2 }?.let { SerializedIrFileFingerprint(it[0], it[1]) }
        }

        fun filePathFingerprint(path: String) = FingerprintHash(cityHash128(path.toByteArray()))

        fun calculateFileFingerprint(file: SerializedIrFile): FingerprintHash {
        ...
        fun calculateFileFingerprint(lib: KotlinLibrary, fileIndex: Int): FingerprintHash {
    ...
    constructor(file: SerializedIrFile) : this(
        filePathFingerprint(file.path),
        calculateFileFingerprint(file)
    )

    override fun toString() = "$pathFingerprint$FILE_HASH_SEPARATOR$fileFingerprint"
}

class SerializedKlibFingerprint(val klibFingerprint: FingerprintHash) {
    companion object {
        private fun List<SerializedIrFileFingerprint>.calculateKlibFingerprint(): FingerprintHash {
            val combinedHash = fold(Hash128Bits(0UL, size.toULong())) { acc, x ->
                acc.combineWith(x.pathFingerprint.hash).combineWith(x.fileFingerprint.hash)
            }
