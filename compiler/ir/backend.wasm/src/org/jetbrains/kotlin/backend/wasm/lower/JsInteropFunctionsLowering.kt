    ...
    private fun IrType.jsToKotlinAdapterIfNeeded(isReturn: Boolean): InteropTypeAdapter? {
        ...
        return if (isNullable()) {
            if (isPrimitive) { //nullable primitive should be checked and adapt to target type
                ...
                val externalToIntAdapter = FunctionBasedAdapter(externRefToPrimitiveAdapter)
                NullOrAdapter(
                    adapter = notNullAdapter?.let { CombineAdapter(it, externalToIntAdapter) } ?: externalToIntAdapter
                )
