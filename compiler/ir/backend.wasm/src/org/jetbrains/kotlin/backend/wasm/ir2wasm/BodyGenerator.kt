...
class BodyGenerator(
    ...
    private fun tryGenerateConstVarargArray(irVararg: IrVararg, wasmArrayType: WasmImmediate.GcType): Boolean {
        ...
        body.buildInstr(WasmOp.ARRAY_NEW_DATA, wasmArrayType, WasmImmediate.DataIdx(resourceIndex))
        return true
    }
    ...
    override fun visitVararg(expression: IrVararg) {
        ...
        if (expression.type.isUnsignedArray()) {
            expression.type.getClass()!!.constructors
                .mapNotNull { it.valueParameters.singleOrNull()?.type?.isArray() }
        }
