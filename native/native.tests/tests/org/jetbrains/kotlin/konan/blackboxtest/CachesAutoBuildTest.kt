...
class CachesAutoBuildTest : AbstractNativeStandaloneTest() {
    @Test
    @TestMetadata("simple")
    fun testSimple() {
        val rootDir = File("$TEST_SUITE_PATH/simple")
        val lib = helper.compileToLibrary(rootDir.resolve("lib"), buildDir)
    ...
    fun testDontCacheUserLib() {
        ...
        val externalLib = helper.compileToLibrary(rootDir.resolve("externalLib"), buildDir.resolve("external"))
        val userLib = helper.compileToLibrary(rootDir.resolve("userLib"), buildDir.resolve("user"), externalLib)
    ...
    private val helper = NativeStandaloneTestHelper(this)
