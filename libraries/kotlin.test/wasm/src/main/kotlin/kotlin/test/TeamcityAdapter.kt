...
internal class TeamcityAdapter : FrameworkAdapter {
    ...
    var scheduleNextTaskAfter: Promise<JsValue>? = null
    inline fun runOrScheduleNext(crossinline block: () -> Unit) {
        if (scheduleNextTaskAfter == null) {
            block()
        } else {
            scheduleNextTaskAfter = scheduleNextTaskAfter!!.finally {
                block()
            }
        }
    }

    inline fun runOrScheduleNextWithResult(crossinline block: () -> Promise<JsValue>?) {
        if (scheduleNextTaskAfter == null) {
            val result = block()
            if (result != null)
    ...
    private val flowId: String = "flowId='wasmTcAdapter'"
            ...
            runOrScheduleNextWithResult {
                ...
                if (result is Promise<*>) {
                    return@runOrScheduleNextWithResult result.then(
                        onFulfilled = { value ->
                            println("-- Test $name fulfilled")
                            MessageType.Finished.report(name)
                            value
                        },
                        onRejected = { e ->
                            println("-- Test $name rejected")
